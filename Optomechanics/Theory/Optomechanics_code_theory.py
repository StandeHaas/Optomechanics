...
print(frequencies)
E = 250e9
frequencies_low = frequency(np.array(alpha_roots),E,rho,L,h)
print(frequencies_low)
E = 325e9
frequencies_high = frequency(np.array(alpha_roots),E,rho,L,h)

frequencies_range = []
for i in range(len(frequencies_high)):
    frequencies_range.append([frequencies_low[i], frequencies_high[i]])
print(frequencies_range)
...
for i in range(len(k_n)):
    ...
    plt.plot(x*100000,u, color[i], label='Mode {}'.format(i))


plt.xlabel(r'Length ($\mu$m)')
plt.ylabel(r'Deflection ($\alpha$ $\cdot$ m)')
plt.ylim(-2,2)
plt.xlim(0,0.0002*100000)
plt.legend()
plt.grid(True)
