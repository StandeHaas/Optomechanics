...
lst_data = []
for i in range(2,6):
    data = pd.read_csv(f'IIIA1\IIIA1_{i}.csv')
    s = data['Second']
    v1 = data['Volt']
    v2 = data['Volt2']
...
for i in range(len(lst_data)):
    ...
    plt.grid()
    plt.savefig(f'Images\IIIA2_{i+2}.eps', format='eps')
    plt.show()
...
plt.clf()
yerr = [3e-6 / np.sqrt(j) for j in lst_j] # this is incorrect but just nice to check
plt.errorbar([2,3,4,5], lst_R, yerr=lst_e, fmt='o',  markersize=5, color='b')
plt.xlim(1.5,5.5)
plt.ylim(0, 7e-6)
plt.ylabel(r'Responsivity (m/V)')
plt.xlabel(r'Voltage (V)')
plt.grid()
plt.savefig('Images\IIIA2_errobar_of_r.eps', format='eps')
plt.show()